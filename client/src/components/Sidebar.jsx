        ...
        <div className={`bg-[#8185B2]/10 h-full p-5 rounded-r-xl overflow-y-scroll text-white ${selectedUser ? "max-md-hidden" : ' '}`}>
            <div className='pb-5'>
                ...
                <div className='bg-[#282142] rounded-full flex items-center gap-2 py-3 px-4 mt-5'>
                    ...
                    <input onChange={()=>setInput(e.target.value)} type="text"className='bg-transparent border-none outline-none text-white
            ...
            <div className='flex flex-col'>
                {filteredUsers.map((user,index)=>(
                    ...
                    'bg-[#282142/50]'}`}>
                        ...
                        {unseenMessages[user._id] > 0 && <p className='absolute top-4 right-4 text-xs h-5 w-5 flex justify-center items rounded-full bg-violet-500/50'>{ unseenMessages[user._id] }</p>}
                    </div>
                ))}
